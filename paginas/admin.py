...
with tab1:
    ...
    with st.form("Registro de goles y jugadores"):
        ...
        with col2a:
            st.header("Equipo 1")
            equipo_A_goles_metidos_seleccion = st.selectbox("Goles a favor", 
                    list(range(0,20)), 
                    index=int(equipo_A_goles_metidos),
                    help='''Goles totales metidos por el equipo 1. 
                    Si juegan jugadores no registrados, 
                    puede no coincidir con la suma de goles de abajo''')
            equipo_A_goles_recibidos_seleccion = st.selectbox("Goles en contra", 
                    list(range(0,20)), 
                    index=int(equipo_A_goles_recibidos),
                    help="Goles totales recibidos por el equipo 1")
            st.divider()
            st.write("Jugadores registrados y goles metidos.")
            st.write("En la columna de la izquierda, añade los jugadores apuntados. A la derecha, los goles.")
            # si el dataframe no tiene 7 filas, añadimos hasta 7
            while equipo_A.shape[0] < 7:
                equipo_A = pd.concat([equipo_A,pd.DataFrame(data = {'jugador':[None], 'goles_metidos': [0]})])                 
            st.data_editor(equipo_A[['jugador','goles_metidos']], hide_index=True, use_container_width=True,
                            column_config ={
                                "jugador": st.column_config.SelectboxColumn(
                                    ...
                                    help='''Haz click sobre una fila y selecciona en 
                                    el desplegable el jugador. Puedes buscar introduciendo texto.'''),
                                "goles_metidos":st.column_config.NumberColumn("Goles", 
                                    default=0, min_value=0, max_value=20, 
                                    required=True, step='int',
                                    help="Introduce un número entero entre 0 y 20")
                                    })
            st.write("Suma de goles = " + str(int(equipo_A['goles_metidos'].sum())))
        with col2b:
            st.header("Equipo 2")
            equipo_B_goles_metidos_seleccion = st.selectbox("Goles a favor", 
                    list(range(0,20)), 
                    index=int(equipo_B_goles_metidos),
                    help='''Goles totales metidos por el equipo 2. 
                    Si juegan jugadores no registrados, 
                    puede no coincidir con la suma de goles de abajo''')
            equipo_B_goles_recibidos_seleccion = st.selectbox("Goles en contra", 
                    list(range(0,20)), 
                    index=int(equipo_B_goles_recibidos),
                    help="Goles totales recibidos por el equipo 2")
            st.divider()
            st.write("Jugadores registrados y goles metidos.")
            st.write("En la columna de la izquierda, añade los jugadores apuntados. A la derecha, los goles.")
            # si el dataframe no tiene 7 filas, añadimos hasta 7
            while equipo_B.shape[0] < 7:
                equipo_B = pd.concat([equipo_B,pd.DataFrame(data = {'jugador':[None], 'goles_metidos': [0]})])    
